...
try {
	...
	if($method === "POST") {
		...
		$profile = new Profile(generateUuidV4(), $profileActivationToken, $requestObject->profileEmail, $profileHash, 0, $profileSalt, $requestObject->profileUserName);
		//insert the profile into the database
		$profile->insert($pdo);
		//compose the email message to send with the activation token
		$messageSubject = "One more step before you can start posting, just confirm your account through your email.";
...
EOF;
		...
		$swiftMessage->setFrom(["wh015k3n@gmail.com" => "Forum"]);
		/**
		...
		// update reply
		$reply->message = "Thank you for creating a profile. One more step before you can start posting, you need to verify your email address";
	} else {
