 ...
 *
 * This class is designed to lay the foundation of the unit tests for Forum. It loads all the database
 ...
 **/
abstract class ForumTest extends TestCase {
	use TestCaseTrait;
	/**
	...
	public final function getDataSet() : QueryDataSet {
		...
		$dataset->addTable("community");
		$dataset->addTable("post");
		$dataset->addTable("comment");
		$dataset->addTable("pvote");
		$dataset->addTable("cvote");
		$dataset->addTable("sponsor");
		return($dataset);
	}
	/**
	...
	public final function getConnection() : Connection {
		...
		if($this->connection === null) {
			...
			$config = readConfig("/etc/apache2/capstone-mysql/forum.ini");
			$pdo = connectToEncryptedMySQL("/etc/apache2/capstone-mysql/forum.ini");
